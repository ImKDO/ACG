...
class FlatIllumination:
    """
    Класс для физически корректного расчета освещенности (Lambertian model для источника и поверхности).
    """

    def __init__(self, screen_width, screen_height, width_res, height_res,
                 light_sources, center, radius):
        self.W = screen_width
    ...
    def diffuse_shading(self, point, light_pos, light_intensity):
        """
        Расчет освещенности по Ламберту с учетом Lambertian источника.
        """
        source_normal = np.array([0.0, 0.0, -1.0])  # Нормаль источника
        surface_normal = np.array([0.0, 0.0, 1.0])  # Нормаль поверхности

        direction_to_point_vec = point - light_pos
        distance_mm = np.linalg.norm(direction_to_point_vec)
        if distance_mm < 1e-9:
            return 0.0

        direction_to_point = direction_to_point_vec / distance_mm
        cos_phi = max(0.0, np.dot(source_normal, direction_to_point))

        distance_m = distance_mm / 1000.0

        I_dir = light_intensity * cos_phi
        E_incident = I_dir / (distance_m ** 2)

        light_dir = -direction_to_point  # От точки к источнику
        cos_theta = max(0.0, np.dot(surface_normal, light_dir))

        brightness = E_incident * cos_theta
        return brightness

    def calculate_single_point(self, screen_x, screen_y):
        """Расчет яркости в конкретной точке экрана (x, y)."""
        point = np.array([screen_x, screen_y, 0.0])
        dist_to_center = np.linalg.norm(point[:2] - self.center[:2])
        ...
        for light_source in self.light_sources:
            ...
            val = self.diffuse_shading(point, light_pos, light_intensity)
    ...
    def calculate_brightness_distribution(self):
        ...
        return brightness_matrix, np.array(mask_values)
...
class SphereVisualizationApp(tk.Tk):
    def __init__(self):
        ...
        self.image_frame = ttk.LabelFrame(
            viz_frame, text="Визуализация", padding="10")
        self.image_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        self.plot_frame = ttk.LabelFrame(
            viz_frame, text="Графики сечений", padding="10")
        self.plot_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.tables_frame = ttk.LabelFrame(
            right_panel, text="Таблицы данных", padding="10")
    ...
    def create_widgets(self, parent):
        row = 0
        ttk.Label(parent, text="Экран (мм)", font=("Arial", 10, "bold")).grid(
        ...
        ttk.Label(parent, text="Разрешение (px)", font=("Arial", 10, "bold")).grid(
        ...
        row += 1
        ttk.Label(parent, text="Источник (мм, Вт/ср)", font=("Arial", 10, "bold")
        ...
        row += 1
        self.add_entry(parent, "Сила излучения (I0):", "i0_var", row, "500")
        row += 1
        ttk.Label(parent, text="Маска (мм)", font=("Arial", 10, "bold")).grid(
            row=row, column=0, columnspan=2, sticky="w", pady=(10, 0))
        row += 1
    ...
    def create_tables_area(self):
        ...
        ttk.Label(frame_stats, text="3.3 Статистика",
                  font=("Arial", 9, "bold")).pack(anchor="w")

        cols_s = ("Param", "Value")
        self.tree_stats = ttk.Treeview(
            frame_stats, columns=cols_s, show="headings", height=5)
        self.tree_stats.heading("Param", text="Параметр")
        self.tree_stats.heading("Value", text="Значение (Вт/м²)")
    ...
    def run_calculation(self):
        try:
            ...
            ls = (float(self.xl_var.get()), float(self.yl_var.get()),
                  float(self.zl_var.get()), float(self.i0_var.get()))

            sc_x, sc_y = float(self.xc_var.get()), float(self.yc_var.get())
            radius = float(self.rad_var.get())

            # Инициализация модели
            model = FlatIllumination(
                W, H, Wres, Hres,
                [ls],
                (sc_x, sc_y),
                radius
            ...
            # Основной расчет
            brightness_matrix, mask_values = model.calculate_brightness_distribution()
            ...
            self.update_tables(model, mask_values, sc_x, sc_y, radius)
        except ValueError:
            messagebox.showerror("Ошибка", "Проверьте введенные числа")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Сбой: {e}")

    def update_tables(self, model, mask_values, cx, cy, radius):
        """
        Обновляет таблицы без проекции (плоская поверхность).
        """
        # Очистка
        for i in self.tree_points.get_children():
            self.tree_points.delete(i)
        for i in self.tree_stats.get_children():
            self.tree_stats.delete(i)
        # --- РАСЧЕТ ПРОЕКЦИИ ---
        # Нет проекции, масштаб 1
        offset = radius * 0.3
        ...
        for name, px, py, s_coord in points:
            ...
            self.tree_points.insert(
                "", "end", values=(name, s_coord, f"{val:.2f}"))
        # Статистика
        if len(mask_values) > 0:
            v_max = np.max(mask_values)
            v_min = np.min(mask_values)
            v_avg = np.mean(mask_values)
        else:
