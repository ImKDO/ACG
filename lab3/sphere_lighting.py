...
освещенной точечными источниками света с использованием модели Блинн-Фонга.
...
class SphereIllumination:
    ...
    def blinn_phong_shading(self, point, normal, light_pos, light_intensity):
        """
        Переписанный метод: расчет через явные косинусы углов.
        
        Реализует формулу: E_incoming = (I0 * cos_alpha) / d^2
        """
        # --- ГЕОМЕТРИЯ ---
        # Вектор от точки на сфере к источнику света
        vec_to_light = light_pos - point
        
        # Квадрат расстояния (d^2 или r^2 + z^2 из вашей формулы)
        dist_sq = np.dot(vec_to_light, vec_to_light)
        distance = np.sqrt(dist_sq)

        if distance < 1e-9:
            return 0

        # Нормализованный вектор направления на свет (L)
        L_dir = vec_to_light / distance

        # --- ШАГ 1: Расчет приходящей освещенности (Формула 2.1) ---
        
        # Определяем ось свечения источника (предполагаем, что лампа светит строго вниз)
        source_axis_down = np.array([0, 0, -1]) 

        # Вектор луча, выходящего из источника (от источника к точке)
        ray_from_source = -L_dir

        # cos_alpha - Косинус угла излучения (между осью лампы и лучом)
        # Геометрически это равно z_L / d
        cos_alpha = max(0, np.dot(ray_from_source, source_axis_down))

        # Освещенность, прилетевшая в точку (E)
        # Формула: E = I0 * cos(alpha) / distance^2
        incoming_E = (light_intensity * cos_alpha) / dist_sq


        # --- ШАГ 2: Отражение от поверхности сферы (Блинн-Фонг) ---

        # cos_theta - Косинус угла падения (между нормалью поверхности и лучом света)
        # Закон Ламберта для поверхности сферы
        cos_theta = max(0, np.dot(normal, L_dir))

        # Диффузная часть (матовость)
        diffuse = self.kd * incoming_E * cos_theta

        # Зеркальная часть (блик)
        # Вектор на наблюдателя
        view_dir = self.observer - point
        view_dir = view_dir / np.linalg.norm(view_dir)

        # Вектор половинного угла (H) для модели Блинна
        half_vector = L_dir + view_dir
        half_norm = np.linalg.norm(half_vector)
        
        if half_norm > 1e-9:
            half_vector = half_vector / half_norm
            # cos_beta - угол для расчета блика
            cos_beta = max(0, np.dot(normal, half_vector))
            specular = self.ks * incoming_E * (cos_beta ** self.n)
        else:
            specular = 0
